...
const DEFAULT_ROLES: Role[] = [
  ...
  },
  {
    ...
    color: 'yellow',
    teamMembers: [
      { id: '12', name: 'Nicole Harris' },
      { id: '13', name: 'Kevin Thompson' },
    ],
    permissions: {
      'billing.reports.view_revenue': false,
      'billing.reports.view_aging': true,
...
const getPermissionCategory = (permissionId: string): PermissionCategory | undefined => {
  return DEFAULT_PERMISSION_CATEGORIES.find(category => 
...
type RolesTabProps = {
  sharedRoles: Role[];
  setSharedRoles: (roles: Role[]) => void;
};

export function RolesTab({ sharedRoles, setSharedRoles }: RolesTabProps) {
  // Use shared roles from parent, or initialize with DEFAULT_ROLES if empty
  const [roles, setRolesInternal] = useState<Role[]>(
    (sharedRoles && sharedRoles.length > 0) ? sharedRoles : DEFAULT_ROLES
  );
  
  // Sync roles to parent whenever they change
  const setRoles = (newRoles: Role[] | ((prev: Role[]) => Role[])) => {
    setRolesInternal(prev => {
      const updated = typeof newRoles === 'function' ? newRoles(prev) : newRoles;
      setSharedRoles(updated); // Update parent state
      return updated;
    });
  };
  ...
  const handleSaveNewRole = (roleData: any) => {
    const newRole: Role = {
      id: `custom-${Date.now()}`,
      ...roleData,
      isDefault: false,
  ...
  const handleSaveEditRole = (roleData: any) => {
    ...
    const updatedRoles = roles.map(role =>
      role.id === editingRole.id
        ? {
            ...
            // Store all the permissions and settings
            roleData: roleData, // Store complete role data for later use
          }
        : role
    );
  ...
  const handleDeleteRole = (roleId: string) => {
    ...
    if (roleToDelete?.isDefault) {
      alert('Cannot delete default system roles. You can only modify their permissions or duplicate them to create a custom version.');
      return;
    }
    ...
    if (roleToDelete && roleToDelete.userCount > 0) {
      const confirmed = window.confirm(
        ...
        `Deleting it will remove their permissions. Are you sure you want to continue?`
      );
      if (!confirmed) return;
    } else {
  ...
  // Render Edit/Create Form (shared component)
  const renderRoleForm = (isEdit: boolean, role?: Role) => (
    <div className="bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 rounded-xl border-2 border-purple-500 dark:border-purple-600 p-6 shadow-lg">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b-2 border-purple-200 dark:border-purple-800">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-purple-600 flex items-center justify-center">
            {isEdit ? <Edit className="w-5 h-5 text-white" /> : <Plus className="w-5 h-5 text-white" />}
          </div>
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
              {isEdit ? `Editing: ${role?.name}` : 'Create New Role'}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {isEdit ? 'Customize role details and permissions' : 'Define a new role with specific permissions'}
            </p>
          </div>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {/* Name Field with conditional rendering for system roles */}
        <div>
        ...
        <div>
          <Label htmlFor="role-color" className="text-sm font-medium">Role Color</Label>
      ...
      {/* Base Permissions On (only for create mode) */}
      {!isEdit && (
        <div className="mb-6">
          <Label htmlFor="based-on" className="text-sm font-medium">Base Permissions On</Label>
      ...
      <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <Button
          onClick={isEdit ? handleUpdateRole : handleSaveNewRole}
  ...
  return (
    <>
      <div className="space-y-6">
        ...
        {!isEditModalOpen && !isCreatingRole && (
        ...
        {/* Edit Role Inline - Show at top when editing */}
        ...
        {/* Create Role Inline - Show at top when creating */}
        {isCreatingRole && (
          <EditRoleInline
        ...
        {!isEditModalOpen && !isCreatingRole && (
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 shadow-sm">
            ...
            {/* Roles Grid / Edit Mode */}
            <div className="p-6">
              {isCreatingRole ? (
                /* Create Mode - Full Area Takeover */
                renderRoleForm(false)
              ) : editingRoleId ? (
                /* Edit Mode - Full Area Takeover */
                (() => {
                  const role = roles.find(r => r.id === editingRoleId);
                  if (!role) return null;
                  return renderRoleForm(true, role);
                })()
              ) : (
                /* Cards Grid View */
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <Card
                            className={cn(
                              "absolute inset-0 border-2 transition-all hover:shadow-lg group p-4 flex flex-col",
                              role.id === 'owner' 
                          ...
                          >
                            ...
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <div className={cn('w-2.5 h-2.5 rounded-full', getRoleColorClasses(role.color).badge)} />
                                  <h3 className="text-sm font-medium text-gray-900 dark:text-gray-100">{role.name}</h3>
                                  {role.isDefault && (
                                    <Badge variant="outline" className="text-[10px] py-0 h-4 border-blue-300 dark:border-blue-700 text-blue-700 dark:text-blue-300">
                            ...
                            {/* User Count */}
                            <div className="flex items-center gap-1.5 mb-2 pb-2 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div className="flex-1 mb-2 min-h-0">
                              {role.teamMembers && role.teamMembers.length > 0 && (
                                <div className="space-y-1">
                                  {role.teamMembers.slice(0, 5).map((member) => (
                                    ...
                                    >
                                      <div className={cn(
                                        ...
                                        getRoleColorClasses(role.color).badge
                                      )}>
                                  ...
                                  {role.teamMembers.length > 5 && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        toggleCardFlip(role.id);
                                      }}
                                      className="flex items-center gap-1.5 text-[11px] text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 pl-6.5"
                                    >
                                      +{role.teamMembers.length - 5} more
                                    </button>
                                  )}
                                </div>
                              )}
                            ...
                            {/* Permissions Summary - Moved to bottom */}
                            <div className="mb-2 mt-auto">
                              <div className="flex items-center justify-between mb-1.5">
                                <span className="text-[10px] text-gray-500 dark:text-gray-400">Permissions</span>
                              ...
                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                <div
                                  className={cn('h-1.5 rounded-full', getRoleColorClasses(role.color).badge)}
                                  style={{ width: `${(Object.values(role.permissions).filter(Boolean).length / Object.keys(role.permissions).length) * 100}%` }}
                                />
                              </div>
                            </div>
                            ...
                            >
                              ...
                              <Button
                                ...
                                className="gap-1 text-[11px] h-7 px-2 text-blue-600 hover:text-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/20"
                              ...
                              {!role.isDefault && (
                                <Button
                                  ...
                                  className="text-red-600 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 gap-1 text-[11px] h-7 px-2"
                                >
                          ...
                          <Card
                            className={cn(
                              "absolute inset-0 border-2 transition-all hover:shadow-lg group p-4 flex flex-col overflow-y-auto",
                              role.id === 'owner' && "border-purple-300 dark:border-purple-700 bg-purple-50/50 dark:bg-purple-900/10",
                          ...
                          >
                            ...
                            <div className="mb-3 pb-2 border-b border-gray-200 dark:border-gray-700">
                              ...
                              <p className="text-[10px] text-gray-600 dark:text-gray-400">All Permissions</p>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto">
                              <div className="grid grid-cols-2 gap-3">
                                {DEFAULT_PERMISSION_CATEGORIES.map((category) => {
                                  const categoryPerms = category.permissions.filter(p => role.permissions[p.id] !== undefined);
                                  if (categoryPerms.length === 0) return null;
                                  
                                  return (
                                    <div key={category.id} className="space-y-1.5">
                                      <h4 className="text-[9px] font-bold uppercase tracking-wide text-gray-700 dark:text-gray-300 flex items-start gap-1.5 mb-1">
                                        <div className="w-3 h-3 flex-shrink-0 [&>svg]:w-3 [&>svg]:h-3">
                                          {category.icon}
                                        </div>
                                        <span className="leading-tight flex-1">{category.name}</span>
                                      </h4>
                                      <div className="space-y-0.5">
                                        {categoryPerms.map((permission) => {
                                          const isEnabled = role.permissions[permission.id];
                                          return (
                                            <div
                                              key={permission.id}
                                            ...
                                            >
                                              <span className={cn(
                                                "flex-shrink-0 mt-0.5 font-bold text-[9px]",
                                                isEnabled ? "text-green-600 dark:text-green-400" : "text-red-500 dark:text-red-400"
                                              )}>
                                                {isEnabled ? '✓' : '✗'}
                                              </span>
                                              <span className={cn(
                                                "leading-tight",
                                                isEnabled ? "text-gray-900 dark:text-gray-100" : "text-gray-500 dark:text-gray-500"
                                              )}>
                                                {permission.name}
                                              </span>
                                            </div>
                            ...
                            {/* Actions */}
                            <div 
                              className="flex items-center gap-2 pt-2 border-t border-gray-200 dark:border-gray-700 mt-3"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <Button
                                variant="outline"
            ...
            {/* Empty State */}
            {!isCreatingRole && !editingRoleId && filteredRoles.length === 0 && (
