...
type ProjectsDocumentsTabProps = {
  project: {
    id: string;
    name: string;
    template: string;
...
export function ProjectsDocumentsTab({ project }: ProjectsDocumentsTabProps) {
  ...
  const [tableViewMode, setTableViewMode] = useState<'table' | 'split'>(() => {
    const saved = localStorage.getItem('projectsDocumentsTabViewMode');
    return (saved === 'split' || saved === 'table') ? saved : 'table';
  });
  const [showLinkedAccounts, setShowLinkedAccounts] = useState(false);
  const [selectedAccountId, setSelectedAccountId] = useState<string>(project.id);
  const [searchQuery, setSearchQuery] = useState('');

  // Persist view mode preference
  useEffect(() => {
    localStorage.setItem('projectsDocumentsTabViewMode', tableViewMode);
  }, [tableViewMode]);
  ...
  // Mock linked accounts data - for projects, we'll use project context
  const mockLinkedAccounts: ClientSummary[] = [
    {
      id: project.id,
      name: project.name,
      type: 'Business', // Default to Business for projects
      documentCount: 7,
      newDocumentCount: 2,
    ...
    {
      id: 'linked-1',
      name: `${project.name} - Related Project`,
      type: 'Business',
      documentCount: 5,
      newDocumentCount: 1,
      mostRecentDate: '2024-11-25',
      linkedAccounts: [project.id],
    },
    {
      id: 'linked-2',
      name: `${project.name} - Client Account`,
      type: 'Individual',
      documentCount: 3,
      newDocumentCount: 0,
  ...
  const currentClientSummary = mockLinkedAccounts.find(c => c.id === project.id) || mockLinkedAccounts[0];
  ...
  const currentAccountDisplayName = useMemo(() => {
    ...
    return selectedAccount?.name || project.name;
  }, [showLinkedAccounts, selectedAccountId, accountsToShow, project.name]);

  // Mock documents data - adapted for project context
    ...
    ...(hasLinkedAccounts ? [
      {
        ...
        clientType: mockLinkedAccounts.find(c => c.id === 'linked-1')?.type || 'Business',
      },
      {
        id: '9',
        ...
        clientType: mockLinkedAccounts.find(c => c.id === 'linked-2')?.type || 'Individual',
      },
    ] : []),
  ]);

  // Document checklist/template - what docs projects should have
  ...
  const accountFilteredDocs = useMemo(() => {
    ...
    // Default: show current project's documents
    return documents.filter((d: Document) => d.clientId === project.id);
  }, [documents, showLinkedAccounts, selectedAccountId, project.id, currentClientSummary]);
  ...
  const renderTableHeader = () => {
    ...
    const showClientColumn = showLinkedAccounts || accountGroups.length > 1 || (selectedAccountId && selectedAccountId !== project.id && !showLinkedAccounts);
  ...
  const renderDocumentRow = (doc: Document) => {
    ...
    return (
      ...
      >
        ...
        {(showLinkedAccounts || accountGroups.length > 1 || (selectedAccountId && selectedAccountId !== project.id && !showLinkedAccounts)) && (
          <td className="px-4 py-4">
            <div className="flex items-center gap-2">
      ...
      <div className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-600">
                Organized project documents in folder structure
              </p>
              <Button 
                variant="outline"
              ...
              <Card className="border-dashed border-2 border-gray-300">
                <CardContent className="pt-12 pb-12 text-center">
                  ...
                  <p className="text-sm text-gray-500 mb-4">
                    Create folder templates to organize project documents
                  </p>
        ...
        {viewMode === 'checklist' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-600">
                Manage required documents for {project.name} • {selectedYear}
              </p>
              <Button 
        ...
        {selectedDoc && (
          <MoveDocumentDialog
            ...
            document={{
              ...
              clientId: selectedDoc.clientId || project.id,
              clientName: selectedDoc.clientName || project.name,
              clientType: selectedDoc.clientType || 'Business',
          ...
          <DialogContent className="max-w-2xl" aria-describedby="folder-templates-description">
            <DialogHeader>
              ...
              <DialogDescription id="folder-templates-description">
                Create and manage folder structures that can be applied to any project
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
          ...
          <DialogContent aria-describedby="add-checklist-item-description">
            <DialogHeader>
              ...
              <DialogDescription id="add-checklist-item-description">
                Add a new required document for this project
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div>
