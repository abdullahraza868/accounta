...
export function DocumentsTab({ client }: DocumentsTabProps) {
  ...
  if (statusFilter === 'reviewing') {
    ...
    filteredDocs = documents.filter(d => d.status === 'reviewing');
  } else if (statusFilter === 'all') {
    // "All" view shows all unreviewed documents + documents from selected year
    const unreviewedDocs = documents.filter(d => d.reviewedDate === null);
    if (selectedYear === 'all') {
      filteredDocs = documents;
    } else {
      // Show unreviewed from all years + reviewed from selected year
      const selectedYearDocs = documents.filter(d => d.year === selectedYear);
    ...
    filteredDocs = selectedYear === 'all' 
      ...
      : documents.filter(d => d.year === selectedYear && d.status === statusFilter);
  }
    ...
    <div className="h-full overflow-y-auto">
      <div className="p-6 space-y-6">
        ...
        <Dialog open={showMoveDialog} onOpenChange={setShowMoveDialog}>
          <DialogContent aria-describedby="move-document-description">
            <DialogHeader>
              <DialogTitle>Move Document to Another User</DialogTitle>
              <DialogDescription id="move-document-description">
                Transfer {selectedDoc?.name} to another team member
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div>
              ...
              <div>
                ...
                <Textarea placeholder="Add a message for the recipient..." />
              </div>
            </div>
            <DialogFooter>
