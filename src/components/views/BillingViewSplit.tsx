/**
 * ⚠️ DUAL VIEW FILE ⚠️
 * BillingViewSplit.tsx ↔️ BillingView.tsx
 * 
 * CRITICAL: When making changes to this file, you MUST also update BillingView.tsx
 * 
 * The ONLY differences between the two files should be:
 * - Table layout (split outstanding/paid tables vs single table)
 * - Pagination state (outstandingCurrentPage/paidCurrentPage vs currentPage)
 * 
...
const DraggableStatCard = ({ config, index, invoices, statusFilter, onFilterChange, moveCard, formatCurrency }: DraggableStatCardProps) => {
  ...
  const totalAmount = invoices
    .filter(inv => {
      ...
      if (filterVal === 'unpaid') return inv.status === 'Unpaid';
      if (filterVal === 'overdue') return inv.status === 'Overdue';
      if (filterVal === 'draft') return inv.status === 'Draft' || inv.status === 'Sent to Client';
...
export function BillingViewSplit({}: BillingViewProps) {
  ...
  // SPLIT VIEW SPECIFIC: Separate pagination for outstanding and paid
  ...
  // Mock invoice data with more business clients - EXACT COPY FROM SINGLE VIEW
  ...
  // SPLIT VIEW SPECIFIC: Split into outstanding and paid, then sort
  ...
  const outstandingInvoices = sortInvoices(filteredInvoices.filter(inv => inv.status === 'Overdue' || inv.status === 'Draft' || inv.status === 'Sent to Client'));
  const paidInvoicesFiltered = sortInvoices(filteredInvoices.filter(inv => inv.status === 'Paid'));

  // Get overdue invoices for bulk send
  const overdueForSending = outstandingInvoices.filter(isOverdueForSending);
  ...
  // Stat Card Configurations - EXACT ORDER FROM SINGLE VIEW
  const statCardConfigs: StatCardConfig[] = [
    ...
    },
    {
      id: 'recent',
      label: 'Recently Paid',
      count: `${recentlyPaidInvoices.length} invoices paid in last 7 days`,
    ...
    },
    {
      ...
      color: 'purple',
      bgColor: 'purple-50',
      getValue: (invoices) => invoices.filter(inv => inv.status === 'Unpaid').length,
      filterValue: 'unpaid'
    },
    {
      id: 'overdue',
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6">
        {/* Header Section - EXACT COPY FROM SINGLE VIEW */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-gray-900 dark:text-gray-100">Billing</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {filteredInvoices.length} invoice{filteredInvoices.length !== 1 ? 's' : ''} ({outstandingInvoices.length} outstanding, {paidInvoicesFiltered.length} paid)
            </p>
          </div>
        </div>

        {/* Stats Cards - Draggable - EXACT COPY FROM SINGLE VIEW */}
        ...
        {/* Toolbar - EXACT COPY FROM SINGLE VIEW */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {Math.ceil(filteredInvoices.length / itemsPerPage) || 1} {Math.ceil(filteredInvoices.length / itemsPerPage) === 1 ? 'page' : 'pages'}
              </span>
              <Select
        ...
        {/* SPLIT VIEW SPECIFIC: Two Tables Section */}
        <div className="space-y-6">
          {/* Outstanding Invoices Table */}
          {outstandingInvoices.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <h3 className="text-gray-900 dark:text-gray-100">Outstanding Invoices</h3>
              ...
              <Card className="border border-gray-200/60 dark:border-gray-700 shadow-sm overflow-hidden mb-4">
                <table className="w-full">
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                    {(() => {
                      const startIndex = (outstandingCurrentPage - 1) * itemsPerPage;
                      const paginatedInvoices = outstandingInvoices.slice(startIndex, startIndex + itemsPerPage);
                      ...
                      return paginatedInvoices.map((invoice, index) => (
                        ...
                        >
                          ...
                          {/* Due / Paid - Smart Display with Tooltip */}
                          <td className="px-4 py-4 w-[160px]">
                            <TooltipProvider>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="cursor-help">
                                    {invoice.status === 'Overdue' ? (
                                      <div>
                                        <div className="text-[10px] text-red-600 dark:text-red-400 uppercase tracking-wide mb-0.5 font-medium">
                                          {(() => {
                                            const dueDate = new Date(invoice.dueDate);
                                            const now = new Date();
                                            const daysOverdue = Math.floor((now.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24));
                ...
                {/* Pagination for Outstanding - Inside Card */}
                <TablePaginationCompact
                  currentPage={outstandingCurrentPage}
                  totalPages={Math.ceil(outstandingInvoices.length / itemsPerPage)}
                  onPageChange={setOutstandingCurrentPage}
                />
              </Card>
            </div>
          )}

          {/* Paid Invoices Table - Subtle Green Theme */}
          {paidInvoicesFiltered.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <CheckCircle className="w-5 h-5 text-green-600 dark:text-green-500" />
                <h3 className="text-gray-900 dark:text-gray-100">Paid Invoices</h3>
              ...
              <div className="bg-green-50/30 dark:bg-green-900/10 rounded-lg border border-green-200/40 dark:border-green-800/40 overflow-hidden">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr style={{ background: 'transparent' }}>
                        <th className="px-6 py-4 pl-10 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[240px]">
                          <button 
                            onClick={() => handleSort('client')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                            Client Name{getSortIcon('client')}
                          </button>
                        </th>
                        <th className="px-4 py-4 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[200px]">
                          <button 
                            onClick={() => handleSort('invoiceNo')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                        ...
                        <th className="px-4 py-4 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[160px]">
                          <button 
                            onClick={() => handleSort('created')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                        ...
                        <th className="px-4 py-4 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[120px]">
                          <button 
                            onClick={() => handleSort('amountDue')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                        ...
                        <th className="px-4 py-4 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[140px]">
                          <button 
                            onClick={() => handleSort('paidVia')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                        ...
                        <th className="px-4 py-4 text-left text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[180px]">
                          <button 
                            onClick={() => handleSort('status')} 
                            className="flex items-center hover:text-green-700 dark:hover:text-green-600 transition-colors"
                          >
                        ...
                        <th className="px-4 py-4 text-center text-xs uppercase tracking-wide text-green-700/70 dark:text-green-600/70 w-[100px]">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="bg-green-50/10 dark:bg-green-900/5 divide-y divide-green-200/30 dark:divide-green-800/30">
                    {(() => {
                      const startIndex = (paidCurrentPage - 1) * itemsPerPage;
                      const paginatedInvoices = paidInvoicesFiltered.slice(startIndex, startIndex + itemsPerPage);
                      
                      return paginatedInvoices.map((invoice, index) => (
                        <tr 
                          key={invoice.id}
                          className="hover:bg-green-100/30 dark:hover:bg-green-900/15 transition-colors"
                        >
                          {/* Client Name */}
                          <td className="px-6 py-4 pl-10">
                            <div className="flex items-center gap-2">
                                ...
                                <div className="absolute left-0 top-10 z-50 hidden group-hover/preview:block pointer-events-none">
                                  <div className="w-64 h-80 bg-white dark:bg-gray-800 rounded-lg shadow-2xl border-2 border-purple-200 dark:border-purple-700 p-4">
                                    <div className="w-full h-full bg-gray-100 dark:bg-gray-900 rounded flex items-center justify-center overflow-hidden">
                                      <div className="text-center p-4">
                                        <div className="text-6xl mb-2">📄</div>
                                        <p className="text-sm text-gray-900 dark:text-gray-100 font-medium">{invoice.invoiceNo}</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{invoice.client}</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400">{formatCurrency(invoice.amountDue)}</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Year: {invoice.year}</p>
                                      </div>
                ...
                {/* Pagination for Paid - Inside Card */}
                <TablePaginationCompact
                  currentPage={paidCurrentPage}
                  totalPages={Math.ceil(paidInvoicesFiltered.length / itemsPerPage)}
