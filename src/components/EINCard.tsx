...
type EINCardProps = {
  ...
  onSave: () => void;
  onCancel: () => void;
  onChange: (ein: string) => void;
};

export function EINCard({ 
  ein, 
  onRequestView, 
  isUnlocked, 
  isEditing,
  ...
  const displayEIN = isUnlocked ? ein : `**-***${ein.slice(-4)}`;
    ...
    <Card className="overflow-hidden border-gray-200 dark:border-gray-700 shadow-sm h-full flex flex-col">
      <CardHeader className="pb-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-gray-900 dark:text-gray-100">Employer Identification Number</h3>
          </div>
          <div className="flex items-center gap-2">
            {!isEditing ? (
      ...
      <CardContent className="p-4 flex-1 flex flex-col">
        {!isEditing ? (
          ...
          <div>
            <Label htmlFor="ein" className="text-xs">Employer Identification Number</Label>
            <div className="flex gap-2 mt-1">
              <Input
                id="ein"
                value={isUnlocked ? ein : ein}
